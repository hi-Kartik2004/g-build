...
// Check if filtered test scores session data exists
if (isset($_SESSION['filteredTestScores'])) {
    $data = $_SESSION['filteredTestScores'];
    // Unset the session data to prevent it from persisting unnecessarily
    unset($_SESSION['filteredTestScores']);
} else {
    // If filtered test scores session data does not exist, get all test scores
    $data = getAllTestScores($_SESSION['user']['email'], $_SESSION['user']['usn']);
}

$editTestScore = isset($_SESSION['editTestScore']) ? $_SESSION['editTestScore'] : null;



$semester_scores = getAllSemesterScores($_SESSION['user']['email'], $_SESSION['user']['usn']);

// Unset the editTestScore session data to prevent it from persisting unnecessarily
unset($_SESSION['editTestScore']);
?>
<section class="dashboard__section">
    <div class="dashboard__section-left">
        <!-- main area to show stuff-->
        <div>
            <h4>Test Score Tracker</h4>
